...
def fetch
_uploaded_image(file_storage):
if not file
_storage or file_storage.filename == "":
return None
try:
data = file
_storage.read()
...
return points
def choose
_background_color(art, bg_override=None):
override
_rgb = parse_
hex
_color(bg_override)
...
to
_
_square(art, tolerance=0.12, bg_override=None):
w, h = art.size
if w == 0 or h == 0:
return art
ratio
_diff = abs(w - h) / max(w, h)
if ratio
diff <= tolerance:
_
return art
bg_
color = choose
_background_color(art, bg_override=bg_override)
square_size = max(w, h)
...
for cy in centers:
for cx in centers:
if (cx == 6 and cy == 6) or (cx == 6 and cy == n - 7) or (cx == n - 7 and cy
== 6):
continue
...
def generate_
branded
_qr(data, art=None, bg_override=None):
...
dot
_scale = get_adaptive_
dot
_scale(complexity)
art
_resized = art.resize((n * BOX, n * BOX), Image.LANCZOS)
canvas.paste(art_resized, (QUIET * BOX, QUIET * BOX), art_resized)
def draw
_dot(x0, y0, x1, y1, scale, color):
...
if matrix[r][c]:
draw
_dot(x0, y0, x1, y1, dot_scale, (*dark_color, 255))
else:
white
_scale = max(0.35, min(0.85, dot_scale * 0.88))
draw
_dot(x0, y0, x1, y1, white_scale, (*light_color, 255))
qpx = QUIET * BOX
...
dome
_qr.paste(qr_img, (qr_x, qr_y), qr_img)
return dome
_qr
def trim
_qr_
...
base = dome
_
_qr.resize((dome_w, dome_h), Image.LANCZOS)
dome
...
data
_
_value = html.escape(data_value or "")
safe
art
data
_
_
_b64 = html.escape(art_
data
_b64 or "")
safe
_bg_
override
_value = html.escape(bg_
override
_value or "")
safe
current
_
_bg_hex = html.escape(current_bg_hex or "#ffffff")
return f"""
<!doctype html>
<html>
<head>
...
57%),
#f4f4f4;
linear-gradient(135deg, transparent 43%, #c33 43%, #c33 57%, transparent
}}
.hue-wrap {{
position: relative;
flex: 1;
...
<input type="text" name="data" required placeholder="Enter QR Data"
value="{safe_
data
_value}"><br><br>
...
<input type="file" id="artfile" name="artfile" accept="image/*"
style="display:none">
<input type="hidden" name="art_
data" id="art
data"
_
value="{safe_
...
<button type="button" id="toggle-bg-tools">Change Background
Color</button>
<span class="bg-current-wrap">
<strong>Current:</strong>
<span class="bg-swatch" id="current_bg_swatch"></span>
<span id="current_bg_label">{safe_
current
_bg_hex}</span>
</span>
</div><div id="bg_
...
<button type="button" class="picker-close"
id="picker_
close
_
btn">×</button>
...
<canvas id="sv
_canvas" width="460" height="360"></canvas>
<div id="sv
_knob"></div>
</div>
<div class="hue-row">
...
<canvas id="hue
_canvas" width="420" height="18"></canvas>
<div id="hue
_knob"></div>
</div>
</div>
<div class="alpha-row">
<button type="button" class="dropper-btn-square"
id="eyedropper_btn">Pick From Image</button>
...
<div class="value-box">
<input type="text" id="bg_override" name="bg_
override"
value="{safe_bg_
override
value or safe
...
<div class="value-box">
<input type="text" id="g_
val" value="255">
<div class="lab">G</div>
</div>
<div class="value-box">
<input type="text" id="b_
val" value="255">
<div class="lab">B</div>
</div>
</div>
...
<button type="button" class="doc-swatch" data-
color="#000000" style="background:#000000;"></button>
<button type="button" class="doc-swatch" data-color="#ffffff"
style="background:#ffffff;"></button>
<button type="button" class="doc-swatch" data-
color="{safe_
current
_bg_hex}" style="background:{safe_
current
_bg_hex};"></
button>
</div>
<div class="apply-row">
<button type="submit" class="apply-btn">Apply Background
Color</button>
</div>
</div>
</div>
...
<img class="mockup-dome" src="data:image/png;base64,
{dome_mockup_b64}">
</div>
</div>
</div>
...
const bgToolsPanel = document.getElementById("bg_
tools
_panel");
const pickerCloseBtn = document.getElementById("picker_
close
_btn");
...
data
_value = (request.form.get("data") or "").strip()
bg_
override
_value = (request.form.get("bg_override") or "").strip()
art
data
_
_b64 = (request.form.get("art_data") or "").strip()
art
_file = request.files.get("artfile")
art = fetch
_uploaded_image(art_file)
if art is None and art
data
b64:
_
_
...
_bg_hex = rgb_
to
_hex(qr_img.convert("RGB").getpixel((5, 5)))
if art is not None:
art
data
_
