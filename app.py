...
def generate_branded_qr(data, art=None, bg_override=None):
    ...
    if light_on_dark:
        ...
        # Finder polarity must agree with the rest of the QR matrix. Mixing a
        # black-on-white finder with light-on-dark timing and data modules
        # breaks the continuous 1:1:3:1:1 finder scan line that detectors use.
        # Keep the rounded styling, but render the layers signal/ground/signal.
...
.profile-name {{ font-size: 25px; font-weight: 800; color:{safe_header_name_color}; }}
.profile-title {{ font-size: 15px; color:{safe_header_title_color}; margin-top: 6px; }}
.actions {{ display:flex; align-items:center; gap:10px; justify-content:center; flex-wrap:wrap; margin-top: 18px; }}
...
#live_buttn_preview .actions {{ display:flex; align-items:center; gap:10px; justify-content:center; flex-wrap:wrap; margin-top: 18px; }}
